...
Date: January 2026
Version: 2.0.0 - 2026-01-24 - Added: USB Persistence "Plug and Communicate" support
...
6. Download tiles (US, Canada, World - ALL automatic)
...
def create_symlinks():
    ...
    # Define symlink mappings: (usb_folder_name, local_destination_path)
    ...
    for src_name, dest_dir in [("tilesets", TILESET_DIR), ("my-maps", PBF_MAP_DIR), ("tiledir", YAAC_TILEDIR), ("navit-maps", NAVIT_MAPS)]:
        usb_dir = usb_path / src_name
        
        # Create the USB directory if it doesn't exist (especially for tiledir)
    ...
    for cat in uncopied:
        try:
            if cat == 'tiles':
                ...
                if usb_dir.exists():
                    TILESET_DIR.parent.mkdir(parents=True, exist_ok=True)
                    if TILESET_DIR.exists() and not TILESET_DIR.is_symlink():
                        backup = TILESET_DIR.with_name(f"tilesets.backup.{int(time.time())}")
                        TILESET_DIR.rename(backup)
                        results.append(f"Backed up: {backup}")
                    if TILESET_DIR.is_symlink():
                        TILESET_DIR.unlink()
                    TILESET_DIR.symlink_to(usb_dir)
                    results.append(f"Linked: {TILESET_DIR} -> {usb_dir}")
                ...
                # OSM maps (my-maps) symlink
                usb_dir = usb_path / "my-maps"
                if usb_dir.exists():
                    PBF_MAP_DIR.parent.mkdir(parents=True, exist_ok=True)
                    if PBF_MAP_DIR.exists() and not PBF_MAP_DIR.is_symlink():
                        backup = PBF_MAP_DIR.with_name(f"my-maps.backup.{int(time.time())}")
                        PBF_MAP_DIR.rename(backup)
                        results.append(f"Backed up: {backup}")
                    if PBF_MAP_DIR.is_symlink():
                        PBF_MAP_DIR.unlink()
                    PBF_MAP_DIR.symlink_to(usb_dir)
                    results.append(f"Linked: {PBF_MAP_DIR} -> {usb_dir}")
                    
                # Navit maps symlink
                usb_navit = usb_path / "navit-maps"
                if usb_navit.exists():
                    NAVIT_MAPS.parent.mkdir(parents=True, exist_ok=True)
                    if NAVIT_MAPS.is_symlink():
                        NAVIT_MAPS.unlink()
                    if not NAVIT_MAPS.exists():
                        NAVIT_MAPS.symlink_to(usb_navit)
                        results.append(f"Linked: {NAVIT_MAPS} -> {usb_navit}")
...
def download_tiles():
    ...
    return render_template('download_tiles.html', files=TILE_FILES, dest_path=dest)
...
def api_download_tile():
    ...
    try:
        subprocess.run(['curl', '-L', '-f', '-o', str(dest_file), f"{TILE_BASE_URL}/{TILE_RELEASE}/{filename}"], check=True, capture_output=True)
...
def api_download_osm():
    ...
    dest_dir.mkdir(parents=True, exist_ok=True)
    url = f"{OSM_BASE_CANADA if country=='canada' else OSM_BASE_USA}/{region}-latest.osm.pbf"
